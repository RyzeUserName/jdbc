...
public class NormalForBeanDao implements Template<DataSource> {
    ...
    public void execute(DataSource object) {
        try {
            ...
            cfg.setDirectoryForTemplateLoading(new File(NormalForBeanDao.class.getResource("/").getPath()));
            cfg.setDefaultEncoding("UTF-8");
            cfg.setTemplateExceptionHandler(TemplateExceptionHandler.RETHROW_HANDLER);
            freemarker.template.Template temp = cfg.getTemplate("daoN.ftl");
            ...
            Optional.ofNullable(tables).ifPresent(a ->
                a.stream().forEach(table -> {
                    ...
                    //生成service
                    try (OutputStream fos = new FileOutputStream(new File(dir, table.getTableServiceName() + "Service.java"))) {
                        Writer out = new OutputStreamWriter(fos);
                    ...
                    //生成serviceImpl
                    try (OutputStream fos = new FileOutputStream(new File(dir, table.getTableServiceName() + "ServiceImpl.java"))) {
                        Writer out = new OutputStreamWriter(fos);
                    ...
                    //生成Controller
                    try (OutputStream fos = new FileOutputStream(new File(dir, table.getTableServiceName() + "Controller.java"))) {
                        Writer out = new OutputStreamWriter(fos);
    ...
    public NormalForBeanDao(String author, String version) {
        this.author = author;
        this.version = version;
    }

    public static NormalForBeanDao build(String author, String version) {
        return new NormalForBeanDao(author, version);
    }
}
